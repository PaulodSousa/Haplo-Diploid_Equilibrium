...
Import vcf data `vcf.data <- vcf2GT("path/to/input.vcf")` 
Create matrix with sites as rows and samples as columns `gt_matrix <- vcf.data$gt_matrix` 
contig/scafold/chromosome names `contigs <- vcf.data$contig_vector` 
site positions `pos <- vcf.data$positions` 
